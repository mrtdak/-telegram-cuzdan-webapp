...
class DecisionLLM:
    ...
    def extract_topics(self, query: str, max_topics: int = 3) -> List[str]:
        """Extract topics - DeepSeek R1 format"""
        prompt = f"""User: Extract main topics from this message (maximum {max_topics}).

Message: "{query}"

Rules:
- Meaningful topics, not long words
- One topic per line
- No irrelevant words"""
...
class HafizaAsistani:
    ...
    async def _process_web_result(self, raw_data: str, query: str, user_input: str) -> str:
        ...
        try:
            process_prompt = f"""User: Extract and clean useful information from web search results.
...
INSTRUCTIONS:
1. Extract ONLY information that answers the user question
2. Remove ads, navigation text, irrelevant content
3. Keep relevant numbers, dates, names
4. Note if data appears wrong/outdated
5. Provide clean, summary info (max 500 chars)
6. If no useful info, write "NO_USEFUL_DATA"
...
CLEAN DATA:"""
    ...
    def _intelligent_decision(self, user_input: str, chat_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            decision_prompt = f"""User:

You are a decision system. Analyze the message and output JSON.

{history_section}MESSAGE: {user_input}

Think step by step:
1. TYPE: greeting/info/technical/religious/math/emotional?
2. CONFIDENCE: Do I know this 90%+?
3. SOURCE: My knowledge or need tool?
...
EXAMPLES:
1) "Hello/Hi" → {{"question_type":"greeting","tool_name":"yok","confidence":"high"}}
2) "What is the weather" → {{"question_type":"weather","tool_name":"hava_durumu","confidence":"high"}}
3) "Allah's power" → {{"question_type":"religious","tool_name":"risale_ara","needs_faiss":true}}
4) "yes interesting" → {{"question_type":"followup","tool_name":"yok"}}
5) "What is Python" → {{"question_type":"general","tool_name":"web_ara","tool_param":"Python programming language"}}

Output your thinking, then JSON:
...
"""
            ...
            # DeepSeek R1 raw thinking output
            print("\n" + "=" * 60)
            print("DEEPSEEK RAW THINKING:")
            print("=" * 60)
            for line in llm_response.split('\n')[:15]:
                print(f"  {line[:80]}")
            print("=" * 60)
...
"""
    ...
    def _prompt_olustur(
        ...
        if tool_result:
            print(f"\n   🔧 PROMPT'A EKLENEN TOOL RESULT ({len(tool_result)} char):\n   {tool_result[:400]}...")
            if tool_name == "web_ara" or tool_name == "wiki_ara":
