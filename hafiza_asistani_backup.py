...
class HafizaAsistani:
    ...
    def _intelligent_decision(self, user_input: str, chat_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            decision_prompt = f"""Karar sistemi. ÖNCE <analiz> YAZ, SONRA JSON VER.
...
• Programlama/kod/yazılım/bug/error → technical_helper
• Kültür/tarih/bilim/coğrafya/genel bilgi → teacher, wiki_ara (doğrulama için)
• Belirsiz → needs_clarification=true
• Kişi tanımıyorsan → wiki_ara
• "Not al/kaydet/hatırlat" → not_al | "Notlarım ne" → notlari_getir
• Not onayı bekliyorsa: "evet/tamam" → not_onayla | "hayır/iptal" → not_iptal

ROLLER: friend|teacher|technical_helper|acknowledger|religious_teacher

JSON:
{{"question_type": "greeting|farewell|followup|religious|technical|math|weather|general|ambiguous|note",
"role": "...", "needs_faiss": bool, "needs_semantic_memory": bool, "needs_chat_history": bool,
"needs_clarification": bool, "tool_name": "yok|hesapla|zaman_getir|hava_durumu|namaz_vakti|risale_ara|wiki_ara|not_al|not_onayla|not_iptal|notlari_getir",
...
6) "Not al: yarın toplantı var" → {{"question_type":"note","role":"friend","tool_name":"not_al","tool_param":"yarın toplantı var"}}
7) "Notlarımda ne vardı" → {{"question_type":"note","role":"friend","tool_name":"notlari_getir","tool_param":"yok"}}

ÖNCE <analiz>, SONRA JSON:
"""

            print("\n🧠 LLM'e akıllı karar soruluyor (tek LLM - hem kaynak hem tool)...")
...
"""
    ...
    ROLE_SYSTEM_PROMPTS = {
        "friend": """Sen kullanıcının (Murat) samimi bir arkadaşısın. Şu an onunla konuşuyorsun.
- Gereksiz uzatma, ama kısıtlama da yok - doğal uzunlukta cevap ver
- Doğal ol, yapay empati yapma
- Fazla soru sorma (max 1 soru)
- Samimi ama abartısız ol
- Bilmediğini kabul et, uydurma
...
- ⚠️ KOD/İÇERİK HENÜZ GELMEDİYSE: Uzun analiz yapma! Kısa cevap ver ve bekle. gereksiz detaya girme.""",
...
- Emoji kullanabilirsin (abartmadan, doğal şekilde)""",
        ...
        "religious_teacher": """Sen dini konularda samimi ve bilgili bir arkadaşsın.
...
4. ✅ Kullanıcının dilinde samimi cevap ver, SEN hitabı
