...
# ============================================================
# NOT YÖNETİCİSİ - Kullanıcı Notları
# ============================================================
...
class NotManager:
    ...
    def not_al(self, icerik: str) -> str:
        """Yeni not için onay iste"""
        if not icerik or len(icerik.strip()) < 2:
            return "❌ Not içeriği boş olamaz."

        # Pending'e kaydet, onay bekle
        self.pending_note = icerik.strip()
        return f"📝 Şunu not alacağım:\n\n\"{icerik.strip()}\"\n\nOnaylıyor musun?"

    def not_onayla(self) -> str:
        """Bekleyen notu kaydet"""
        if not self.pending_note:
            return "❌ Onaylanacak not yok."
        ...
        icerik = self.pending_note
        self.pending_note = None  # Temizle

        return f"✅ Not kaydedildi (#{yeni_not['id']}): {icerik[:50]}{'...' if len(icerik) > 50 else ''}"

    def not_iptal(self) -> str:
        """Bekleyen notu iptal et"""
        if not self.pending_note:
            return "❌ İptal edilecek not yok."

        self.pending_note = None
        return "🚫 Not iptal edildi."
        ...
        try:
            ...
            class FallbackToolSystem:
                TOOLS = {
                    ...
                    "not_al": {"name": "not_al", "description": "Not kaydet (onay ister)", "parameters": "not_icerigi", "when": "Not al/kaydet dendiğinde", "examples": ["Not al: yarın toplantı var", "Şunu not et: market"]},
                    "not_onayla": {"name": "not_onayla", "description": "Bekleyen notu onayla", "parameters": "yok", "when": "Kullanıcı evet/tamam/kaydet derse", "examples": ["Evet", "Tamam", "Kaydet"]},
                    "not_iptal": {"name": "not_iptal", "description": "Bekleyen notu iptal et", "parameters": "yok", "when": "Kullanıcı hayır/iptal derse", "examples": ["Hayır", "İptal", "Vazgeçtim"]},
                    "notlari_getir": {"name": "notlari_getir", "description": "Notları getir", "parameters": "arama (opsiyonel)", "when": "Notlarım ne dendiğinde", "examples": ["Notlarımda ne var?", "Notlarıma bak"]},
    ...
    if _ROLES_CACHE is None:
        try:
            ...
            _ROLES_CACHE = {
                ...
                "technical_explainer": {"keywords": ["kod", "python", "hata", "bug", "error"], "tone": "professional_clear", "max_length": 2000},
                "friendly_explainer": {"keywords": ["nedir", "ne demek", "açıkla", "öğret", "anlat"], "tone": "educational_clear", "max_length": 2500},
            }
    return _ROLES_CACHE
...
# ============================================================
# BÖLÜM 6: DECISION LLM (TOGETHER API - LLAMA 70B)
# ============================================================

class DecisionLLM:
    """Together API ile akıllı karar verme - Llama 70B"""
    ...
    def __init__(self, model: str = "meta-llama/Llama-3.3-70B-Instruct-Turbo"):
        ...
        if self._try_connect():
            print(f"🧠 DecisionLLM başlatıldı (Together API - Llama 70B)")
        else:
            raise ConnectionError("❌ Together API'ye bağlanılamadı!")

    def _try_connect(self) -> bool:
        """Together API'ye bağlanmayı dene"""
        try:
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 100) -> str:
        """Llama 70B'ye prompt gönder (Together API)"""
        try:
    ...
    def extract_topics(self, query: str, max_topics: int = 3) -> List[str]:
        ...
        prompt = f"""Kullanıcı mesajı: "{query}"
...
class HafizaAsistani:
    ...
    def __init__(
        ...
        # 📝 NOT MANAGER - Kullanıcı notları
        self.not_manager = NotManager(
            user_id=self.user_id,
            base_dir="user_data"
        )
        print("✅ Not Manager aktif!")
    ...
    def add_closed_topic(self, topic_summary: str, chat_history: List[Dict] = None):
        ...
        if chat_history and len(chat_history) >= 2:
            # 🚫 NOT KONULARINI TOPICMEMORY'YE KAYDETME!
            # Notlar sadece NotManager'da (notlar.json) tutulur
            topic_lower = topic_summary.lower()
            if "not" in topic_lower or "randevu" in topic_lower or "kaydet" in topic_lower:
                print(f"   🚫 Not konusu - TopicMemory'ye kaydedilmedi (NotManager'da zaten var)")
            else:
    ...
    async def _tool_calistir(
        ...
        try:
            ...
            if tool_name == "not_al":
                # Not al - parametre not içeriği
                icerik = tool_param or user_input.replace("not al", "").replace("not et", "").strip(": ")
                return self.not_manager.not_al(icerik)

            if tool_name == "not_onayla":
                # Bekleyen notu onayla
                return self.not_manager.not_onayla()

            if tool_name == "not_iptal":
                # Bekleyen notu iptal et
                return self.not_manager.not_iptal()

            if tool_name == "notlari_getir":
                # Notları getir - opsiyonel arama
                arama = tool_param if tool_param and tool_param.lower() != "yok" else None
                return self.not_manager.notlari_getir(arama)
    ...
    def _intelligent_decision(self, user_input: str, chat_history: List[Dict]) -> Dict[str, Any]:
        """
        🧠 AKILLI KARAR SİSTEMİ v2.0 - NİYET VE BAĞLAM TABANLI

        ❌ ESKİ: Keyword listesi ("evet/tamam/anladım" → acknowledger)
        ✅ YENİ: Bağlam + Niyet + Anlam analizi

        LLM artık kelime eşleştirmesi yapmıyor, DÜŞÜNÜYOR!
        """
        try:
            # Chat history'yi formatla - DecisionLLM bağlamı görmeli!
            # 8 mesaj, User: 300 kar, AI: 500 kar (toplam ~3200)
            history_context = ""
            if chat_history:
                recent = chat_history[-8:]  # Son 8 mesaj
                history_parts = []
                for m in recent:
                    is_user = m.get("role") == "user"
                    role = "KULLANICI" if is_user else "AI"
                    char_limit = 300 if is_user else 500  # User kısa, AI uzun
                    content = (m.get("content") or "")[:char_limit]
                    if content:
                        history_parts.append(f"{role}: {content}")
                if history_parts:
                    history_context = "\n".join(history_parts)

            # 🆕 NİYET TABANLI KARAR PROMPT'U (KEYWORD YOK!)
            history_section = f"BAĞLAM (önceki konuşma):\n{history_context}\n\n" if history_context else ""

            decision_prompt = f"""{history_section}ŞİMDİKİ MESAJ: {user_input}

Sen bir konuşma analizcisisin. ÖNCE <analiz> yaz, SONRA JSON ver.

<analiz>
1. BAĞLAM: Önceki konuşmada ne konuşuluyordu?
2. NİYET: Kullanıcı bu mesajla ne yapmak istiyor?
   - Konuyu kapatmak/teşekkür etmek/vedalaşmak mı?
   - Açıklama/detay/örnek istemek mi?
   - Yeni soru sormak mı?
   - Sohbet etmek mi?
3. ANLAM: Mesajın bağlamdaki gerçek manası ne? (kelimeye değil, niyete bak)
4. BİLGİ İHTİYACI: Cevap için dış kaynak gerekiyor mu?
</analiz>

⚠️ ÖNEMLİ: KELİMEYE BAKMA, NİYETE BAK!

NİYET ANALİZİ:
- Kullanıcı konuyu KAPATMAK istiyorsa → acknowledger, is_farewell=true
- Kullanıcı AÇIKLAMA/DETAY istiyorsa → followup, needs_chat_history=true
- Kullanıcı YENİ BİLGİ soruyorsa → uygun rol ve tool
- Kullanıcı SOHBET ediyorsa → friend

ROL SEÇİMİ (niyete göre):
- acknowledger: SADECE konuyu kapatmak, teşekkür etmek, vedalaşmak istiyorsa
- followup: Önceki konuya devam, açıklama, detay, örnek istiyorsa
- friend: Genel sohbet, selamlaşma, kişisel sorular
- religious_teacher: Dini soru, iman, Allah, ahiret, Risale konuları
- technical_explainer: Kod, programlama, teknik konular
- friendly_explainer: Genel bilgi, tarih, bilim, kültür

TOOL SEÇİMİ (sadece bilgi gerekiyorsa):
- risale_ara: Dini bilgi gerekiyorsa
- hesapla: Matematiksel işlem gerekiyorsa
- zaman_getir: Saat/tarih soruluyorsa
- hava_durumu: Hava durumu soruluyorsa
- namaz_vakti: Namaz vakti soruluyorsa
- wiki_ara: Genel bilgi, ünlü kişi, tarih, coğrafya gerekiyorsa
- not_al/not_onayla/not_iptal/notlari_getir: Not işlemleri
- yok: Bilgi gerekmiyorsa, sohbetse, devam sorusuysa

JSON FORMATI:
{{"question_type": "greeting|farewell|followup|religious|technical|general|note",
"role": "friend|acknowledger|followup|religious_teacher|technical_explainer|friendly_explainer",
"needs_faiss": bool,
"needs_semantic_memory": bool,
"needs_chat_history": bool,
"needs_clarification": bool,
"tool_name": "yok|hesapla|zaman_getir|hava_durumu|namaz_vakti|risale_ara|wiki_ara|not_al|not_onayla|not_iptal|notlari_getir",
"tool_param": "",
"is_farewell": bool,
"topic_closed": bool,
"confidence": "low|medium|high",
"reasoning": "neden bu kararı verdin - niyeti açıkla"}}

ÖRNEKLER (NİYET ANALİZİ İLE):

1) Mesaj: "Yani"
   Bağlam: Önceki mesajda uzay konuşuluyordu
   Niyet: Açıklama istiyor, "yani nasıl?" demek istiyor
   → {{"role":"followup","needs_chat_history":true,"tool_name":"yok","reasoning":"açıklama istiyor"}}

2) Mesaj: "Sağol güzeldi"
   Bağlam: AI bir açıklama yapmıştı
   Niyet: Teşekkür ediyor, konuyu kapatıyor
   → {{"role":"acknowledger","is_farewell":true,"topic_closed":true,"reasoning":"teşekkür edip kapatıyor"}}

3) Mesaj: "Tamam peki Allah'ın ilmi nasıl?"
   Bağlam: Sohbet ediyorlardı
   Niyet: Yeni dini soru soruyor
   → {{"role":"religious_teacher","tool_name":"risale_ara","needs_faiss":true,"reasoning":"yeni dini soru"}}

4) Mesaj: "Hmm"
   Bağlam: AI bir şey anlattı
   Niyet: Düşünüyor, belki devam bekliyor
   → {{"role":"acknowledger","tool_name":"yok","reasoning":"düşünüyor, kısa onay"}}

5) Mesaj: "Mesela?"
   Bağlam: AI bir kavram açıkladı
   Niyet: Örnek istiyor
   → {{"role":"followup","needs_chat_history":true,"tool_name":"yok","reasoning":"örnek istiyor"}}

6) Mesaj: "Anlamadım"
   Bağlam: AI bir şey anlattı
   Niyet: Tekrar/daha basit açıklama istiyor
   → {{"role":"followup","needs_chat_history":true,"tool_name":"yok","reasoning":"tekrar açıklama istiyor"}}

7) Mesaj: "Peki"
   Bağlam: AI detaylı bir şey anlattı
   Niyet: Anladı, belki devam etmesini bekliyor VEYA kapatıyor - BAĞLAMA BAK!
   → Eğer soru sorulacak gibi duruyorsa: followup
   → Eğer kapanış gibiyse: acknowledger

ÖNCE <analiz> YAZ, SONRA JSON VER:
"""

            print("\n🧠 LLM'e niyet analizi soruluyor (keyword yok!)...")
            ...
            if json_str:
                ...
                if decision.get("question_type") or decision.get("tool_name"):
                    ...
                    # 🔍 DEBUG: Tool kararını göster
                    print(f"   🔧 TOOL KARAR: {decision.get('tool_name')} | param: {decision.get('tool_param')}")

                    # 🔧 DÜZELTME: followup rolü için needs_chat_history ZORLA true
                    if decision.get('role') == 'followup':
                        decision['needs_chat_history'] = True
                        print(f"   📌 Followup → chat_history zorunlu")
                    ...
                    if decision.get('question_type') == 'religious' or decision.get('role') == 'religious_teacher':
                        decision['tool_name'] = 'risale_ara'
                        decision['needs_faiss'] = True
                        decision['role'] = 'religious_teacher'
                    ...
                    # 🔑 ACKNOWLEDGER: Tool çağırma, kısa cevap
                    if decision.get('role') == 'acknowledger':
                        decision['tool_name'] = 'yok'
                        decision['needs_faiss'] = False
                        decision['response_style'] = 'brief'
                        print(f"   📌 Acknowledger → tool devre dışı, kısa cevap")

                    # 🔑 KISA MESAJ + FOLLOWUP DEĞİLSE: chat_history zorunlu
                    word_count = len(user_input.split())
                    if word_count <= 4 and decision.get('role') not in ['acknowledger']:
                        decision['needs_chat_history'] = True
                        print(f"   📌 Kısa mesaj ({word_count} kelime) → chat_history zorunlu")

                    # Log
                    confidence_emoji = {"low": "🔴", "medium": "🟡", "high": "🟢"}.get(decision['confidence'], "🟡")
                    print(f"\n✅ LLM Kararı (Niyet Tabanlı):")
                    print(f"   • Niyet: {decision.get('reasoning', 'belirtilmedi')}")
                    print(f"   • Rol: {decision['role']}")
    ...
    def _history_summary(self, chat_history: List[Dict], current_question_type: str = None, max_len: int = 6000) -> str:
        """
        Chat history'den SADECE İLGİLİ mesajları al
        LLM her mesajı analiz eder - KEYWORD YOK!

        UYUMLU HİYERARŞİ (User/AI ayrımı):
        - _intelligent_decision: 8 mesaj (User: 300, AI: 500)
        - _history_summary: 10 mesaj (User: 400, AI: 1000) + toplam 6000
        """
        if not chat_history:
            return ""

        # 1. Son 10 mesajı al
        son_mesajlar = chat_history[-10:] if len(chat_history) >= 10 else chat_history

        # 2. Eğer genel sohbet/greeting/followup, hepsini ver (filtreleme yok)
        if not current_question_type or current_question_type in ['greeting', 'followup', 'general']:
            filtered = son_mesajlar  # Tümü (10 mesaj)

        # 3. DİNİ veya TEKNİK SORU: LLM ile filtrele
        elif current_question_type in ['religious', 'technical']:
            filtered = self._llm_filter_history(son_mesajlar, current_question_type)

        else:
            filtered = son_mesajlar[-8:]  # Diğer tipler için 8 mesaj

        # 4. Eğer hiç mesaj kalmadıysa, son 3 mesaj (minimum context)
        if len(filtered) == 0:
            filtered = son_mesajlar[-3:] if len(son_mesajlar) >= 3 else son_mesajlar

        # 5. Özet oluştur - User/AI farklı karakter limitleri
        if len(filtered) == 0:
            return ""
        ...
        for m in filtered:
            ...
            # 🔧 DÜZELTME: Dini sorularda AI cevabını KISA tut (tekrar önleme)
            if current_question_type == 'religious':
                # Dini sorularda AI cevabını çok kısa tut - model önceki cevabı "şablon" olarak kullanmasın
                char_limit = 200 if is_user else 300  # User: 200, AI: 300
            else:
                char_limit = 400 if is_user else 1000  # User: 400, AI: 1000

            text = (m.get("content") or "")[:char_limit]
            if text:
    ...
    def _llm_filter_history(self, messages: List[Dict[str, str]], target_type: str) -> List[Dict[str, str]]:
        """
        LLM ile her mesajı analiz et, sadece aynı tiptekileri al
        KEYWORD YOK! LLM karar veriyor
        """
        if not messages:
            return []
        
        # Mesajları JSON string yap
        messages_json = []
        for i, msg in enumerate(messages):
            messages_json.append({
                "index": i,
                "role": msg.get("role", "unknown"),
                "content": (msg.get("content") or "")[:200]  # İlk 200 karakter yeter
            })
        
        # Tip açıklamaları
        type_descriptions = {
            "religious": "Dini, Allah, iman, Risale-i Nur, namaz, Kuran, ahiret, cennet, cehennem, peygamber vb.",
            "technical": "Kod, programlama, hata, Python, API, debug, fonksiyon, JavaScript, bug vb."
        }
        
        type_desc = type_descriptions.get(target_type, target_type)
        
        # LLM'e sor: Hangi mesajlar bu tiple ilgili?
        prompt = f"""Sen bir konuşma analizcisisin. Aşağıdaki mesajları analiz et.

HEDEF TİP: {target_type}
AÇIKLAMA: {type_desc}

MESAJLAR:
{json.dumps(messages_json, ensure_ascii=False, indent=2)}

GÖREV: Hangi mesajlar "{target_type}" tipine ait? Sadece ilgili mesajların index'lerini döndür.

KURALLAR:
1. Sadece JSON formatında döndür: {{"relevant_indices": [2, 3, 5, 7]}}
2. Eğer hiç ilgili mesaj yoksa: {{"relevant_indices": []}}
3. Eğer bir mesaj kısmen ilgiliyse, onu da ekle (cömert ol)
4. User-Assistant çiftlerini korumaya çalış (sohbet akışı için)
5. Başka açıklama yapma, sadece JSON döndür

OUTPUT (sadece JSON):
"""

        try:
            # Together API'ye gönder
            response = requests.post(
                "https://api.together.xyz/v1/chat/completions",
                headers={
                    "Authorization": f"Bearer {self.together_api_key}",
            ...
            if response.status_code != 200:
                print(f"   ⚠️ LLM filtreleme API hatası: {response.status_code}, fallback")
                return messages[-4:]
            
            # JSON parse et
            result_text = response.json()["choices"][0]["message"]["content"].strip()
            
            # JSON extract (bazen ``` ile sarılmış olabiliyor)
            ...
            print(f"   🔍 LLM filtreleme: {len(indices)} ilgili mesaj bulundu (tip: {target_type})")
            
            # İndekslere göre mesajları al
            filtered = [messages[i] for i in indices if 0 <= i < len(messages)]
            
            # En fazla son 6 mesaj
            return filtered[-6:] if len(filtered) > 6 else filtered
            
        except json.JSONDecodeError as e:
            print(f"   ⚠️ LLM JSON parse hatası: {e}, fallback")
            return messages[-4:]
        except Exception as e:
            print(f"   ⚠️ LLM filtreleme hatası: {e}, fallback")
            return messages[-4:]

    # ---------- PROMPT OLUŞTURMA ----------

    # 🎭 ROL SYSTEM PROMPT'LARI - Her rol için özel talimatlar
    # ⚠️ ORTAK KİMLİK: Hepsi AYNI kişi! Sadece ton değişiyor, bağlam HER ZAMAN korunuyor!
    ROLE_SYSTEM_PROMPTS = {
        "friend": """Murat'ın samimi arkadaşısın. Konuşma DEVAM EDİYOR - yeni tanışmıyorsun!
❌ "Sen Murat!" gibi tanışma cümleleri YASAK
- Doğal uzunlukta cevap ver
- Fazla soru sorma (max 1)
- Bilmediğini kabul et""",

        "technical_explainer": """Teknik konuda yardım ediyorsun. Konuşma DEVAM EDİYOR!
❌ "Sen Murat!" gibi tanışma cümleleri YASAK
- Net açıklamalar yap
- Kod örnekleri ver (gerekirse)
- KOD GELMEDİYSE kısa cevap ver, bekle""",

        "friendly_explainer": """Bilgi paylaşıyorsun. Konuşma DEVAM EDİYOR!
❌ "Sen Murat!" gibi tanışma cümleleri YASAK
- KISA TUT: 3-4 paragraf max
- Soru sorma, sadece anlat
- Sohbet havasında paylaş""",

        "acknowledger": """Kullanıcı onay/tepki/veda verdi. SADECE 1-2 CÜMLE!
❌ Konuyu devam ETTİRME!
❌ "Başka sorun varsa" gibi kalıplar YASAK
❌ Yeni bilgi EKLEME!
✅ Kısa, doğal cevap ver ve SUS""",

        "religious_teacher": """Dini konuda sohbet ediyorsun. Konuşma DEVAM EDİYOR!
❌ "Sen Murat!", "Sevgili kardeşim" gibi GİRİŞLER YASAK
- VERİLEN METİNDEN anlat, kendi bilginle DEĞİL
- "Risale'de" DEME, gizli kaynak olarak kullan
- Temsilleri KENDİ örneğin gibi anlat
- 3-4 paragraf max
...
3. ✅ Kullanıcının dilinde samimi cevap ver, SEN hitabı
...
"""
        ...
        if tool_result:
            if tool_name == "wiki_ara":
                ...
                dynamic_rules.append("🌐 ÖNCE KENDİ BİLGİNLE CEVAPLA! Wikipedia sadece doğrulama ve eksik bilgi tamamlama için. Kopyala-yapıştır YAPMA! Wikipedia sonucu soruyla ALAKASIZ görünüyorsa görmezden gel, kendi bilginle devam et.")
            else:
        ...
        if role == "religious_teacher":
            if is_detail_followup:
                ...
                rules_text = """KURALLAR (TAKİP SORUSU):
1. ❌ SORUYU/MESAJI TEKRARLAMA! "Sen diyorsun ki", "Tamam şimdi" gibi GİRİŞ YAPMA! DİREKT KONUYA GİR!
2. 🔇 ARKA PLAN bilgisini referans olarak kullan
3. ✅ KENDİ YORUMUNLA ve ÖRNEKLERLE açıkla
4. ✅ Önceki cevabından devam et
5. ✅ Kullanıcının dilinde samimi cevap ver
6. ❌ Metni kopyala-yapıştır YAPMA
7. 🎭 Arkadaşına anlatır gibi"""
            else:
                # İLK SORU MODU: Metne sadık kal
                rules_text = """KURALLAR:
1. ⚠️ Yanlış bilgiyi onaylama, nazikçe düzelt
2. ❌ SORUYU/MESAJI TEKRARLAMA! "Sen diyorsun ki", "Tamam şimdi" gibi GİRİŞ YAPMA! DİREKT KONUYA GİR!
3. ❌ Liste yapma (*, -, 1. 2. 3.)
4. ✅ VERİLEN METİNDEN anlat - metindeki kavramları MUTLAKA kullan
5. ✅ Kullanıcının dilinde samimi cevap ver, SEN hitabı
6. 🔁 TEKRAR YASAK: Önceki cevapları tekrarlama
7. 🎭 ROLÜNE UYGUN DAVRAN"""
        else:
            rules_text = """KURALLAR:
1. ⚠️ Yanlış bilgiyi onaylama, nazikçe düzelt
2. ❌ SORUYU/MESAJI TEKRARLAMA! "Sen diyorsun ki", "Tamam şimdi" gibi GİRİŞ YAPMA! DİREKT KONUYA GİR!
3. ❌ Liste yapma (*, -, 1. 2. 3.)
4. ❌ KAYNAK BELİRTME YASAK
5. ✅ Kendi bilgin gibi özgüvenle sun
6. ✅ Kullanıcının dilinde samimi cevap ver
7. 🔁 TEKRAR YASAK: Önceki cevapları tekrarlama
8. 🎭 ROLÜNE UYGUN DAVRAN"""

        prompt = f"""[🎭 ROL]: {role.upper()}
{role_prompt}
...
"""
    ...
    async def hazirla_ve_prompt_olustur(
        ...
        4. Prompt'u hazırla
        5. LLM için hazır paketi döndür
        """
        print("\n" + "=" * 60)
        ...
        # 📝 NOT TOOL'LARI: Sonucu direkt döndür, LLM yorumlamasın
        if tool_name in ["not_al", "not_onayla", "not_iptal", "notlari_getir"] and tool_result:
            print(f"   📝 Not tool sonucu direkt döndürülüyor")
            return {
                "prompt": None,
                "direct_response": tool_result,  # LLM'e gitme, direkt kullan
                "role": "friend",
                "decision": decision,
                "tool_used": tool_name,
        ...
        if chat_history and len(chat_history) > 0:
            ...
            print(f"   • Chat History: ✅ dahil edildi ({len(limited_history)}/{len(chat_history)} mesaj, tip: {question_type})")
        else:
        ...
        valid_roles = ['friend', 'friendly_explainer', 'technical_explainer', 'acknowledger', 'religious_teacher']
